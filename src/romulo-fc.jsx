...
const DEMO_PLAYERS = [
  { id:9001, nombre:"Carlos",   apellido:"Demo",      cat:"Sub-15", num:10, tel:"04140000001", repNombre:"Pedro", repApellido:"Demo", repTel:"04140000002", repCedula:"V-99000001", cedula:"V-88000001", dob:"2010-03-15", stats:{ goles:8, asistencias:5, partidos:12 } },
  { id:9002, nombre:"Andrés",   apellido:"Ejemplo",   cat:"Sub-13", num:7,  tel:"04140000003", repNombre:"Ana",   repApellido:"Ejm",  repTel:"04140000004", repCedula:"V-99000003", cedula:"V-88000003", dob:"2012-06-22", stats:{ goles:4, asistencias:7, partidos:10 } },
  { id:9003, nombre:"Luis",     apellido:"Prueba",    cat:"Sub-17", num:1,  tel:"04140000005", repNombre:"María", repApellido:"Prb",  repTel:"04140000006", repCedula:"V-99000005", cedula:"V-88000005", dob:"2008-11-08", stats:{ goles:0, asistencias:2, partidos:14 } },
];
const CAT_COLOR = {
...
export default function App() {
          ...
          onSave={r => {
            ...
            if (liveM.champId && liveM.fase === "Normal") {
              ...
              if (ch && ch.fase === "grupos") {
                ...
                setDoc(doc(db,"champs",String(liveM.champId)), { ...ch, standings });
              }
            }
            ...
            if (r.convocados && r.convocados.length > 0) {
              ...
              setDoc(doc(db,"att_matches",matchAttId), {
                matchId: liveM.id,
                date: liveM.date,
                cat: liveM.cat,
                home: liveM.home,
            ...
            if (r.mvp) {
              const matchData2 = { ...liveM, scoreH:r.scoreH, scoreA:r.scoreA, status:"finalizado",
                events: r.events||[], playerStats: r.playerStats||{},
                mvp: r.mvp };
              setDoc(doc(db,"matches",String(liveM.id)), matchData2);
            }
  ...
  if (!isAdmin) {
    ...
    const SPEC_TABS = isVisitor
      ...
      : [["inicio","🏠","Mi Perfil"],["pagos","💳","Pagos"],["campeonatos","🏆","Tabla"],["partidos","📅","Partidos"],["entrenos","🏃","Entrenos"],["torneos","🌍","Torneos"],["chat","💬","Chat"]];
    ...
    function renderSpecContent() {
      ...
      if (tab === "chat") {
        ...
        const CANALES = [miCat, "General"].filter((v,i,a) => a.indexOf(v)===i);
        ...
        return (
          <>
            ...
            <div className="dtabs" style={{ marginBottom:10 }}>
              {CANALES.map(c => (
                <div key={c} className={"dt"+(chatCat===c?" da":"")} onClick={() => setChatCat(c)}>{c}</div>
              ))}
            </div>
            ...
            <div style={{ display:"flex", gap:7 }}>
              <textarea className="inp" style={{ flex:1, resize:"none", minHeight:40, lineHeight:1.5, paddingTop:9 }}
                ...
                onKeyDown={e => { if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault();
                  ...
                  setDoc(doc(db,"chat",id),{ id, canal:chatCat, texto:txt, autor:user?.name||"Jugador", rol: role==="player"?"Jugador":"Representante", ts:new Date().toISOString(), uid:user?.playerId||"" });
                  setChatMsg("");
                }}}
              />
  ...
  function renderAdminContent() {
              ...
              <div className="modal">
                ...
                <div className="inp-wrap">
                  ...
                  <div style={{ fontSize:7.5, color:"#3a5068", marginTop:3 }}>
                    Solo para referencia visual al armar el equipo. No restringe en qué partido puede jugar.
                  </div>
                </div>
                <div className="div" />
    ...
    if (tab === "torneos") {
      const torneos = clubConfig?.torneos || [];
      const [showTForm, setShowTForm]   = React.useState(false);
      const [editTIdx,  setEditTIdx]    = React.useState(null);
      const T_BLANK = { nombre:"", fecha:"", lugar:"", costo:"", cats:[], resultado:"", notas:"" };
      const [nt, setNt] = React.useState(T_BLANK);

      function saveTorneo() {
        if (!nt.nombre || !nt.fecha) return;
        const nuevos = [...torneos];
        if (editTIdx !== null) { nuevos[editTIdx] = nt; }
        else { nuevos.push({ ...nt, id: Date.now() }); }
        setDoc(doc(db,"config","club"), { ...clubConfig, torneos: nuevos });
        setShowTForm(false); setEditTIdx(null); setNt(T_BLANK);
      }
      ...
      return (
        <>
          <div className="st">🌍 Torneos Externos</div>
          {can("config") && (
            <button className="btn" style={{ marginBottom:9 }} onClick={()=>{ setNt(T_BLANK); setEditTIdx(null); setShowTForm(true); }}>
              + Registrar Torneo
            </button>
          )}
            ...
            return (
              <div key={t.id||idx} className="card" style={{ marginBottom:8 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
                  ...
                  {can("config") && (
                    <div style={{ display:"flex", gap:4 }}>
                      <button className="btn-sm" onClick={()=>{ setNt({...t}); setEditTIdx(idx); setShowTForm(true); }}>✏️</button>
          ...
          {showTForm && (
            <div className="ov" onClick={e=>{ if(e.target.className==="ov"){ setShowTForm(false); setEditTIdx(null); }}}>
              <div className="modal">
                <div className="mt2">{editTIdx!==null?"✏️ Editar Torneo":"Nuevo Torneo Externo"}
                  <span className="mx" onClick={()=>{ setShowTForm(false); setEditTIdx(null); }}>✕</span></div>
                <div className="inp-wrap">
                  <div className="inp-lbl">Nombre del Torneo *</div>
                  <input className="inp" placeholder="Ej: Copa Carabobo 2026" value={nt.nombre}
                    onChange={e=>setNt(n=>({...n,nombre:e.target.value}))}/>
                </div>
                <div className="inp-2">
                  <div className="inp-wrap">
                    <div className="inp-lbl">Fecha *</div>
                    <input className="inp" placeholder="15 Mar 2026" value={nt.fecha}
                      onChange={e=>setNt(n=>({...n,fecha:e.target.value}))}/>
                  </div>
                  <div className="inp-wrap">
                    <div className="inp-lbl">Lugar</div>
                    <input className="inp" placeholder="Ciudad o cancha" value={nt.lugar}
                      onChange={e=>setNt(n=>({...n,lugar:e.target.value}))}/>
                  </div>
                </div>
                <div className="inp-2">
                  <div className="inp-wrap">
                    <div className="inp-lbl">Costo inscripción ($)</div>
                    <input className="inp" type="number" placeholder="0" value={nt.costo}
                      onChange={e=>setNt(n=>({...n,costo:e.target.value}))}/>
                  </div>
                  <div className="inp-wrap">
                    <div className="inp-lbl">Categorías</div>
                    <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginTop:3 }}>
                      {CATS.map(cat=>(
                        <button key={cat} onClick={()=>setNt(n=>({...n,cats: n.cats.includes(cat)?n.cats.filter(x=>x!==cat):[...n.cats,cat]}))}
                          className={"btn-sm"} style={{ fontSize:8, padding:"3px 6px",
                            background: nt.cats.includes(cat)?"rgba(21,101,192,.25)":"rgba(255,255,255,.03)",
                            color: nt.cats.includes(cat)?"#7ab3e0":"#4e6a88" }}>
                          {cat}
                        </button>
                      ))}
                    </div>
                ...
                <div className="inp-wrap">
                  ...
                  <textarea className="inp" rows={2} placeholder="Ej: Subcampeones, eliminados en cuartos..."
                    value={nt.resultado} onChange={e=>setNt(n=>({...n,resultado:e.target.value}))}
                    style={{ resize:"none" }}/>
                </div>
    ...
    if (tab === "uniformes") {
      ...
      function saveUniforme(pid, data) {
        const pl = players.find(x => x.id === pid);
        if (!pl) return;
        setDoc(doc(db,"players",String(pid)), { ...pl, uniforme: { ...(pl.uniforme||{}), ...data } });
      }
                  ...
                  onClick={() => setConf({
                    ...
                    ok: () => {
                      if (!champActivo) {
                        ...
                        players.forEach(p => {
                          setDoc(doc(db,"pay",String(p.id)), { id:p.id, months:{}, history:[], arbitraje:[] });
                          setDoc(doc(db,"sanc",String(p.id)), { id:p.id, yellows:0, reds:0, suspended:false });
                          const upd = { ...p, stats:{ goles:0, asistencias:0, partidos:0 } };
                          setDoc(doc(db,"players",String(p.id)), upd);
                        });
                      }
                      const nuevaTemp = parseInt(tempActual)+1;
                      setDoc(doc(db,"config","club"), { ...clubConfig, temporada: nuevaTemp,
  ...
  return (
    <>
      ...
      {surveyTarget && (() => {
        ...
        const [svSentido, setSvSentido]     = React.useState(0);
        const [svPractico, setSvPractico]   = React.useState("");
        const [svMolestia, setSvMolestia]   = React.useState("no");
        const [svZona, setSvZona]           = React.useState("");
        const [svProximo, setSvProximo]     = React.useState("si");
        ...
        function saveSurvey() {
          ...
          setDoc(doc(db,"surveys",pid+"_"+session), surveyDoc);
      ...
      {galeriaModal && (() => {
        ...
        function subirFoto(e) {
          ...
          rdr.onload = ev => {
            ...
            setDoc(doc(db,"matches",String(match.id)), { ...match, fotos: nuevas });
          };
          rdr.readAsDataURL(f);
        }
      ...
      {mvpModal && (() => {
        ...
        function castVoto(playerId) {
          ...
          setDoc(doc(db,"matches",String(match.id)), updated);
        }
                    ...
                    {!votacionCerrada && (
                      <button className="btn-sm" style={{ fontSize:8 }}
                        onClick={() => {
                          ...
                          setDoc(doc(db,"matches",String(match.id)), upd);
                        }}>Cambiar</button>
                    )}
                  </div>
                ) : null;
