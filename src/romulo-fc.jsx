...
export default function App() {
  ...
  // ── Torneos estados (sacados de dentro del tab para evitar hook condicional) ──
  const T_BLANK_G = { nombre:"", fecha:"", lugar:"", costo:"", cats:[], resultado:"", notas:"" };
  const [showTForm, setShowTForm] = useState(false);
  const [editTIdx,  setEditTIdx]  = useState(null);
  const [nt,        setNt]        = useState(T_BLANK_G);
  // ── Encuesta estados ──
  ...
  function renderAdminContent() {
    ...
    if (tab === "torneos") {
      ...
      function saveTorneo() {
        ...
        setShowTForm(false); setEditTIdx(null); setNt(T_BLANK_G);
      }
      ...
      return (
        <>
          <div className="st">🌍 Torneos Externos</div>
          {can("config") && (
            <button className="btn" style={{ marginBottom:9 }} onClick={()=>{ setNt(T_BLANK_G); setEditTIdx(null); setShowTForm(true); }}>
              + Registrar Torneo
            </button>
          )}
    ...
    if (tab === "entrenamientos") {
      ...
      function saveTrain() {
        ...
        setNt(NT_BLANK_T); setShowTForm(false); setFormErr(""); setEditTrain(null);
      }
      ...
      return (
        <>
          <div className="st">🏃 Entrenamientos</div>
          {can("jugadores") && (
            <button className="btn" style={{ marginBottom:9 }} onClick={()=>{ setShowTForm(true); setEditTrain(null); setNt(NT_BLANK_T); setFormErr(""); }}>
              + Agregar Sesión
            </button>
          )}
            ...
            <div className="ov" onClick={e=>{ if(e.target.className==="ov"){ setShowTForm(false); setEditTrain(null); } }}>
              <div className="modal">
                <div className="mt2">{editTrain?"✏️ Editar Sesión":"Nueva Sesión de Entrenamiento"}
                  <span className="mx" onClick={()=>{ setShowTForm(false); setEditTrain(null); }}>✕</span></div>
